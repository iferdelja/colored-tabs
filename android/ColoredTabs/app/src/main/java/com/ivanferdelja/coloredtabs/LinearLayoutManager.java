...
public class LinearLayoutManager extends android.support.v7.widget.LinearLayoutManager {
	...
	public void onMeasure(RecyclerView.Recycler recycler, RecyclerView.State state, int widthSpec, int heightSpec) {
		...
		//!!!! For some reason I keep getting true here, instead i want to do the measuring myself.
		final boolean exactHeight = false;// heightMode == View.MeasureSpec.EXACTLY;
			...
			if (vertical) {
				if (!hasChildSize) {
					if (i < stateItemCount) {
						...
						measureChild(recycler, i, widthSize, unspecified, childDimensions, i == (adapterItemCount - 1));
					} else {
						logMeasureWarning(i);
					}
				}
				...
				if (!hasChildSize) {
					if (i < stateItemCount) {
						// we should not exceed state count, otherwise we'll get IndexOutOfBoundsException. For such items
						// we will use previously calculated dimensions
						measureChild(recycler, i, unspecified, heightSize, childDimensions, i == (adapterItemCount - 1));
					} else {
						logMeasureWarning(i);
					}
				}
	...
	private void measureChild(RecyclerView.Recycler recycler, int position, int widthSize, int heightSize,
							  int[] dimensions, boolean isLastChild) {
		final View child;
		try {
		...
		// !!!!!! DO NOT MEASURE bottom decoration for LAST CHILD !!!!!
		if (isLastChild) {
			vDecoration = getTopDecorationHeight(child);
		}
